...
function validateAndPay(a) {
   ...
   */
    ...
    if (isEmailIsChecked == true && isFirstNameIsChecked == true && isLastNameChecked == true && isNumberChecked == true && isPurposeChecked == true && isAmountCheck == true) {
        ...
        localStorage['TSchl_purpose'] = purpose; // only strings 
        localStorage['TSchl_firstname'] = fname; // only strings
        localStorage['TSchl_lastname'] = lastname; // only strings
        localStorage['TSchl_amountm'] = amountm; // only strings
        localStorage['TSchl_PhoneNumber'] = localStorage['TSchl_countrydialCode'] + "" + PhoneNumber; // only strings
        localStorage['TSchl_student_email'] = emailValue; // only strings
